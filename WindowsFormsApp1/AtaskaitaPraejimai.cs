...
namespace PCMSystem
{
    public partial class AtaskaitaPraejimai : Form
    {
        public AtaskaitaPraejimai()
        {
            InitializeComponent();
        ...
        private void button_filtruoti_Click(object sender, EventArgs e)
        {
            ...
            int searchId = Convert.ToInt32(dfind_dropbox_idList.SelectedItem);
            string praejimas = Program.gateRepository.GetGateCodeByID(searchId);
            ...
            else if (check_filter_true.Checked == true)
            {
                foreach (var item in listToReport)
                {
                    if (item.GateId == searchId && item.Pass == true)
                    {
                        UzpildytiAtaskaita(ref eilNr, ref rowNr, item);
                    }
                }
            ...
            else if (check_filter_false.Checked == true)
            {
                foreach (var item in listToReport)
                {
                    if (item.GateId == searchId && item.Pass == false)
                    {
                        UzpildytiAtaskaita(ref eilNr, ref rowNr, item);
                    }
                }
        ...
        private void UzpildytiAtaskaita(ref int eilNr, ref int rowNr, Event item)
        {
            ...
            Ataskaita_dataGridView.Rows[rowNr].Cells[1].Value = gateId;
            Ataskaita_dataGridView.Rows[rowNr].Cells[2].Value = gate;
